...
// does). If unset, the SDK falls back to its own search.
const CLAUDE_BIN = process.env.CLAUDE_CODE_EXECUTABLE?.trim();
...
export async function switchSession(
  ...
  // The UI folds the change marker into the user's next message itself
  // (same shape as the bootstrap merge) — see InputBox.fire(). switchSession
  // itself stays silent in the transcript.
...
export async function resumeAllTracked(): Promise<void> {
  ...
  for (const entry of valid) {
    ...
    try {
      ...
      console.log(`resumed session ${entry.sessionId}`);
    } catch (err) {
