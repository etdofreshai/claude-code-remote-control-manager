...
export const ccrcEndpoints: HelpEndpoint[] = [
  ...
  {
    method: "POST",
    path: "/api/sessions/resolve-remote",
    auth: "bearer",
    description: "Resolve a Claude.ai code URL or session_ id to a local client/session candidate using stored mappings or Claude.ai metadata matched against knownSessions.",
    body: { url: "Claude.ai /code/session_... URL", remote: "Alternative session_... or cse_... id", client: "Optional client filter" },
  },
  {
    method: "POST",
    path: "/api/sessions/resume-by-remote",
    auth: "bearer",
    description: "Resolve a Claude.ai code URL/session_ id, then resume the matched local Claude Code session on its client. Returns candidates instead of guessing when ambiguous.",
    body: { url: "Claude.ai /code/session_... URL", remote: "Alternative session_... or cse_... id", client: "Optional client filter", cwd: "Optional cwd override", name: "Optional session title override" },
  },
  { method: "POST", path: "/api/clients/:name/sessions/:sessionId/message", auth: "bearer", description: "Send a text message to a running client-managed session. This steers the session but does not cancel an in-progress tool call." , body: { text: "Required message text" } },
  { method: "POST", path: "/api/clients/:name/sessions/:sessionId/interrupt", auth: "bearer", description: "Hard-interrupt a running local Claude Code session, resume the same local UUID, and re-enable remote control. Optional body text is sent after resume." , body: { text: "Optional steering message sent after interrupt/resume", name: "Optional refreshed session title" } },
  { method: "POST", path: "/api/clients/:name/sessions/:sessionId/interrupt-and-message", auth: "bearer", description: "Hard-interrupt a running local Claude Code session, resume it, then send a required steering message." , body: { text: "Required steering message sent after interrupt/resume", name: "Optional refreshed session title" } },
  ...
  { method: "GET", path: "/api/agent/ws", auth: "bearer", description: "Preferred client WebSocket transport. Client connects with name query param and bearer auth; server pushes commands, client sends ack/session/disconnect messages over the socket." },
  { method: "POST", path: "/api/agent/connect", auth: "bearer", description: "Legacy HTTP long-poll client connect endpoint." },
  { method: "POST", path: "/api/agent/sessions", auth: "bearer", description: "Client reports local sessions discovered on its machine." },
  { method: "GET", path: "/api/agent/poll", auth: "bearer", description: "Legacy HTTP client long-polls for queued commands." },
  { method: "POST", path: "/api/agent/ack", auth: "bearer", description: "Legacy HTTP client acknowledges a command result." },
  ...
  { method: "GET", path: "/api/claude-ai/sessions/:sessionId/events", auth: "bearer", description: "Proxy one Claude.ai session event-history page; supports limit and after_id exactly like Claude.ai." },
  { method: "GET", path: "/api/claude-ai/sessions/:sessionId/events/all", auth: "bearer", description: "Fetch all Claude.ai event-history pages by following has_more/last_id with after_id; returns raw event objects in Claude.ai shape plus pages." },
  { method: "GET", path: "/api/claude-ai/sessions/:sessionId/messages", auth: "bearer", description: "Fetch all event-history pages and return only user/assistant message events." },
